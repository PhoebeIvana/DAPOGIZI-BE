...
// Geocode a free-text address → { lon, lat, formatted } | null
...
// Internal helper to call Places with given params
async function fetchPlaces({ categories, bias, limit, radius }) {
  const url = new URL("https://api.geoapify.com/v2/places");
  url.searchParams.set("categories", categories);            // e.g. education.school / education
  url.searchParams.set("limit", String(limit || 3));
  url.searchParams.set("lang", "id");
  url.searchParams.set("apiKey", API_KEY);
  url.searchParams.set("filter", "countrycode:id");

  // Use a circle filter to ensure results around the vendor point
  if (bias?.lon != null && bias?.lat != null && radius) {
    url.searchParams.set("filter", `circle:${bias.lon},${bias.lat},${radius}`);
    // bias still helps ranking inside the circle
...
async function findNearbySchools(bias, limit = 3) {
  ...
  for (const step of steps) {
    const list = await fetchPlaces({
      ...
      bias,
      limit: Math.max(limit, 6), // fetch a few more, we’ll sort & slice
      radius: step.radius,
    });
    collected = collected.concat(list);
    if (collected.length >= limit) break; // we have enough candidates
  }

  // de-duplicate by geoapify_id (or name+coords if id missing)
  const seen = new Set();
  const deduped = [];
